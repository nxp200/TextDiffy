/* TextDiffy Application - IIFE build for file:// compatibility */
(function(){
  'use strict';
  ...
  async function buildStructuredDiff(textA, textB, opts=AppState){
  ...
  function renderDiff(structured){
  ...
  function updateMetrics(structured){
    ...
    const addedEl = document.getElementById('total-added');
    const removedEl = document.getElementById('total-removed');
    const modifiedEl = document.getElementById('total-modified');
    if(addedEl) addedEl.textContent = String(add);
    if(removedEl) removedEl.textContent = String(rem);
    if(modifiedEl) modifiedEl.textContent = String(mod);
  }

  // ---------- Serialization ----------
  function serializePlain(structured){
    const lines=[];
    for(const item of structured){
  ...
  function bindUI(){
    ...
    if(ws) ws.addEventListener('change', ()=>{ AppState.whitespaceSensitive = ws.checked; });
    if(cs) cs.addEventListener('change', ()=>{ AppState.caseSensitive = cs.checked; });
    if(gran) gran.addEventListener('change', ()=>{ AppState.granularity = gran.value; });
    const compareBtn = $('#compareBtn');
    const copyBtn = $('#copyBtn');
    const exportBtn = $('#exportBtn');
    if(compareBtn) compareBtn.addEventListener('click', async ()=>{ await runCompare(); });
  ...
  // ---------- Expose API for tests / extensibility ----------
  window.TextDiffy = {
    AppState,
    buildStructuredDiff,
    renderDiff,
